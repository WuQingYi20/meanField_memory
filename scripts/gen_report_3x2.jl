...
println(io, "Without experiential learning (None row), convergence rate is 0/100 at all N, regardless of normative memory. With normative memory on, the final majority reaches ~0.59 (vs ~0.53 without) -- a modest improvement from within-tick transient signals, but far below the 0.95 convergence threshold. **Norms can amplify existing patterns but cannot create them from scratch.**")
println(io)
