...
\citet{bicchieri2006grammar} distinguished empirical expectations (``what is done'') from normative expectations (``what should be done''). All reviewed models use a single representation---a belief distribution, a propensity vector, or a utility parameter---that conflates the descriptive and the prescriptive: agents converge behaviourally but do not undergo a discrete internal state transition from ``I observe $X$'' to ``$X$ is the rule.'' Young's and Shoham \& Tennenholtz's agents converge without forming rules; Boyd \& Richerson's agents copy the majority without distinguishing ``most do $X$'' from ``one should do $X$.'' \citet{gavrilets2017collective} model internalisation explicitly through a genetic parameter~$\eta$, elegantly explaining the \emph{ultimate} cause, but the \emph{proximate} question---how an agent within a single generation transitions from ``no norm'' to ``internalised rule''---requires a cognitive process, not a fixed trait. Bridging this distance requires (i)~architecturally separating statistical beliefs from normative rules, and (ii)~modelling the \emph{transition process} by which the former gives rise to the latter.
...
$\Phi$ (\emph{SocialPressure}) & $[0, \infty)$ & Gain multiplier for partner-directed enforcement signals (Section~\ref{sec:enforcement_signal}). $\Phi = 0$: all normative signalling is disabled---no enforcement signals are sent or received. $\Phi = 1$: baseline enforcement. $\Phi > 1$: amplified social pressure. \\
\bottomrule
\end{tabular}
\end{center}
...
\node[step, fill=normyellow]   (s5) at (0,-4.8)  {5.\ Partner-Directed Enforcement ($\Phi$)};
...
\node[var] at (5.8,-4.8)  {\textbf{Write:} $p_j$ to partner (Eq.~\ref{eq:signal_push}) \quad \textbf{Gate:} $\Phi$};
\node[var] at (5.8,-6.0)  {\textbf{Read:} all; no state written};
\end{tikzpicture}
\caption{Single-tick dataflow. Left: execution stages in fixed order; environment gates ($V$, $\Phi$) annotated where they modulate information flow. Right: variables written/read at each stage, with equation references. Enforcement signals (Stage~5) are directed to the matched interaction partner only, not broadcast globally. Use this figure to trace where any symbol first appears and where it is consumed.}
...
Predictive confidence is \textbf{not} a parameter of either memory system. It is a single state variable whose semantic interpretation naturally produces three functional roles in the pre-crystallisation phase:

\begin{center}
\adjustbox{max width=\linewidth}{\begin{tabular}{lp{4cm}p{5.5cm}}
\toprule
\textbf{Confidence level} & \textbf{Meaning} & \textbf{Consequence (pre-crystallisation)} \\
\midrule
Low ($< 0.3$) & ``I cannot predict this environment'' & Short window; high DDM drift rate; high signal sensitivity \\
Medium ($0.3$--$0.7$) & ``I can roughly predict'' & Moderate window; moderate DDM drift rate \\
High ($> 0.7$) & ``I can reliably predict'' & Long window; low DDM drift rate; low signal sensitivity \\
\bottomrule
\end{tabular}}
\end{center}

\noindent Post-crystallisation compliance is governed exclusively by norm strength $\sigma_i$ (Section~\ref{sec:decision}), not by $C_i$. The roles listed above are therefore pre-crystallisation only.
...
\draw[arrow, red!70, dashed] (normmem) -- node[label, right, xshift=2pt] {if $\Phi\!>\!0$,\;$\sigma\!>\!\theta$\\+ violation\\{\scriptsize to partner only}} (signal);
...
\node[draw=lockinpink, fill=lockinpink, rounded corners, text width=12.5cm, align=center, font=\small] at (2, -2) {
    ...
    \textbf{L3} (Normative): consistency $\to$ DDM crystallisation $\to$ norm $\to$ compliance $\to$ partner enforcement $\to$ gradual cascade
};

\end{tikzpicture}
\caption{Dual-memory architecture. Predictive confidence bridges experience memory (statistical, FIFO) and normative memory (rule-based, anomaly-tracked). Observations ($V$ per tick beyond the direct partner) feed both systems simultaneously. Enforcement signals are \emph{partner-directed}: when agent~$i$ observes its matched partner~$j$ violating $i$'s norm, agent~$i$ sends a signal to~$j$ only (not to the population). Signals are violation-triggered, norm-strength-gated, and scaled by $\Phi$. When $V = 0$ or $\Phi = 0$, the corresponding channel is disabled (see Section~\ref{sec:ablation}).}
...
\begin{center}
\small
\texttt{Experience layer} $\;\xrightarrow{\;C_i\;}\;$ \texttt{Normative layer} $\;\xrightarrow{\;\sigma_i\;}\;$ \texttt{Decision layer} $\;\xleftarrow{\;p_i\;}\;$ \texttt{Partner Enforcement}
\end{center}

\begin{table}[h]
\centering
\small
\adjustbox{max width=\linewidth}{\begin{tabular}{lp{3.8cm}p{4.5cm}p{3.5cm}}
\toprule
...
$p_i$ & Partner-directed enforcement (Eq.~\ref{eq:violation_response}) & DDM evidence update (Eq.~\ref{eq:ddm}) & ``My partner says I should do $X$'' \\
\bottomrule
\end{tabular}}
...
Prior to each interaction, agent $i$ forms a prediction $\hat{s}_j$ of its partner's strategy using the \textbf{MAP (maximum a posteriori) rule}: $\hat{s}_j = \arg\max_{s} b_s^{\text{eff}}$. A prediction is \emph{correct} when $\hat{s}_j = s_j$ (the partner's actual choice), regardless of agent $i$'s own action or the resulting payoff.

\paragraph{Why MAP prediction, not probabilistic?} Action selection uses probability matching (Eq.~\ref{eq:prob_matching}), but prediction uses the deterministic MAP rule. The two serve different functions: action selection is a \emph{behavioural} choice where probability matching provides neutral exploration (no best-response bias), but prediction is an \emph{internal assessment} of environmental regularity that should reflect the agent's best estimate. Using probabilistic prediction would conflate exploration noise with perceived environmental volatility, artificially suppressing confidence even when beliefs are accurate. In pure coordination games, prediction accuracy and coordination success are highly correlated, but they are conceptually independent: an agent may predict correctly yet fail to coordinate (due to stochastic action selection), or coordinate by chance despite an incorrect prediction.
...
\noindent Under MAP prediction, the probability of a correct prediction equals $\max(b_A, b_B)$, so Eq.~\ref{eq:confidence_steady} becomes:
\begin{equation}\label{eq:confidence_steady_map}
C^* = \frac{\max(b_A, b_B)\,\alpha}{\max(b_A, b_B)\,\alpha + \min(b_A, b_B)\,\beta}
\end{equation}
At $\mathbf{b} = [0.5, 0.5]$: $C^* = \alpha / (\alpha + \beta) = 0.25$ (with defaults). At $\mathbf{b} = [0.7, 0.3]$: $C^* \approx 0.44$. At $\mathbf{b} = [0.95, 0.05]$: $C^* \approx 0.86$. The lock-in threshold---the belief level at which $C^*$ is high enough to sustain a long memory window---is thus a derived quantity, not a free parameter.
...
The chain from cold start to norm has two stages, each with a distinct causal mechanism.

\paragraph{Stage 1: Symmetry breaking (cold start $\to$ behavioural majority).}
At $t = 0$, all agents hold uniform beliefs $\mathbf{b}^{\text{exp}} = [0.5, 0.5]$ and select strategies by probability matching (Eq.~\ref{eq:prob_matching}), producing a 50-50 population split. In a finite population, random pairing creates sampling noise: agent $i$'s memory window of size $w_i$ contains a \emph{sample}, not the population distribution. A window of size 4 that happens to contain 3$A$ and 1$B$ produces $b_A = 0.75$, making agent $i$ play $A$ with 75\% probability. If this prediction aligns with the partner's action---more likely as the local majority grows---predictive confidence $C_i$ increases (Eq.~\ref{eq:confidence_update}), the memory window expands (Eq.~\ref{eq:window}), and the majority-biased sample is retained longer. The asymmetry $\beta > \alpha$ ensures that agents who bet on the minority strategy lose confidence faster, shortening their windows and making them susceptible to flipping. This positive feedback amplifies initial sampling fluctuations into a stable behavioural majority.

\begin{center}
Sampling noise $\to$ local majority in window $\to$ probability matching favours majority $\to$\\
correct predictions $\to$ $C \!\uparrow$ $\to$ window $\uparrow$ $\to$ majority belief retained $\to$ behavioural consistency
\end{center}

Probability matching is behaviourally neutral (no best-response bias): the symmetry is broken by \emph{finite-sample noise amplified by the confidence--memory feedback}, not by a behavioural rule favouring coordination. \textbf{Limitation}: without social amplification ($V = 0$), convergence is slow because each agent's information is limited to its own interaction history.

\paragraph{Stage 2: From behavioural majority to norm (lock-in $\to$ DDM crystallisation).}
Once Stage~1 produces a behavioural majority (e.g., 70\% play $A$), the population provides a \emph{consistent signal} to the normative memory's DDM. The signed consistency $f_A - f_B \approx 0.4$ generates non-zero drift (Eq.~\ref{eq:drift}). Crucially, the agents most susceptible to norm formation are precisely those who have \emph{not} locked in: their predictive confidence $C_i$ is low, so their DDM drift rate $(1 - C_i)(f_A - f_B)$ is high. These uncertain agents crystallise norms first (H2), begin complying (Eq.~\ref{eq:effective}), and thereby \emph{increase} the population consistency that drives further DDM accumulation in remaining agents. When $V > 0$, social observations amplify this signal; when $\Phi > 0$, early norm-holders who observe their partner violating the norm send a directed enforcement signal to that partner (Eq.~\ref{eq:signal_push}), pushing the partner's DDM evidence accumulator toward crystallisation. This is the handoff from Loop~1 (individual lock-in) through Loop~2 (social amplification) to Loop~3 (normative pressure).

\begin{center}
Behavioural majority (Stage~1) $\to$ $f_A \!-\! f_B \neq 0$ $\to$ DDM drift in low-$C$ agents $\to$ crystallisation $\to$\\
compliance $\uparrow$ consistency $\to$ more crystallisation $\to$ enforcement $\to$ cascade
\end{center}
...
\paragraph{Why does a norm form, and what does the model claim?} A norm's \emph{content}---which strategy becomes ``the rule''---is induced from observed behavioural frequencies: the DDM drift $(1 - C_i)(f_A - f_B)$ is a function of the descriptive statistics of the population. In this sense, the model does not avoid the is--ought transition \citep{hume1739treatise}; it models it explicitly as a psychologically grounded inductive process \citep{sherif1936psychology,deutsch1955study}. The mechanism is the interaction between meta-cognitive uncertainty and observed social consistency:
\begin{quote}
    ``They are so consistent, they must know something I don't. That must be the rule.''
\end{quote}

What the architecture \emph{does} provide is a structural distinction between the two sides of this transition. Before crystallisation, the agent holds a continuously updated statistical belief ($\mathbf{b}^{\text{exp}}$); after crystallisation, the agent holds a discrete rule ($r_i$) maintained by anomaly tracking, not statistical updating. The is--ought gap is not bridged by avoiding induction from experience, but by ensuring that the inductive product---the norm---is stored, updated, and acts on decisions in a qualitatively different way from the beliefs that gave rise to it (constraint vs.\ source, anomaly-tracked vs.\ FIFO, compliance-governed vs.\ probability-matched). This is an \emph{architectural} claim about cognitive representation, not an epistemological claim about the derivability of norms from experience.
...
The design rationale is that norm erosion under sustained violations is non-linear: isolated anomalies are tolerated, but a burst of contradictory evidence triggers a discrete drop in conviction rather than gradual decay \citep{centola2018tipping}.
...
\textbf{Symbol} & \textbf{Range} & \textbf{Defined in} & \textbf{Role} \\
\midrule
$p_i$ & $\mathbb{R}$ & This section & Partner-directed signal push (\textbf{bridge} $\to$ normative DDM) \\
\midrule
...
Norm enforcement is \textbf{reactive}, \textbf{local}, and \textbf{partner-directed}. Agent $i$ sends a normative signal \emph{to its matched interaction partner $j$} only when three conditions hold simultaneously:

\begin{enumerate}[itemsep=2pt]
    \item The agent \textbf{has a norm} ($r_i \neq \varnothing$)
    \item The agent has \textbf{high norm strength} ($\sigma_i > \theta_{\text{enforce}}$)
    \item The agent \textbf{observes its partner violating} the norm ($s_j \neq r_i$)
\end{enumerate}
...
\text{Observe partner } s_j \neq r_i: \quad \begin{cases}
\Phi > 0 \;\text{and}\; \sigma_i > \theta_{\text{enforce}}: & \text{signal}(r_i) \to j \quad \textit{(enforce partner)} \\
...
\paragraph{Why partner-directed, not broadcast?} In a well-mixed population, a global broadcast would function as a hidden global coordinator: a single enforcing agent could simultaneously push \emph{all} uncommitted agents toward crystallisation, bypassing the local interaction structure. This would conflate enforcement strength with population reach, making $\Phi$ difficult to interpret. Partner-directed signalling preserves locality: enforcement influence propagates only through pairwise encounters, at the same rate as behavioural information. Cascading norm adoption therefore requires \emph{many} enforcers encountering \emph{many} violators over \emph{many} ticks, not a single agent broadcasting to the population in one tick.

\paragraph{Effect of $\Phi = 0$.} When social pressure is disabled ($\Phi = 0$), the enforcement channel is closed: no agent sends normative signals, regardless of norm strength. Violations that would otherwise trigger enforcement instead accumulate as anomalies (the ``otherwise'' branch of Eq.~\ref{eq:violation_response}). This makes norms more fragile without social reinforcement---a testable prediction of the model (cf.\ H4).

\subsection{Effect of Normative Signals on Receivers}\label{sec:enforcement_signal}

When agent $j$ receives a normative signal from its matched partner $i$, the signal delivers a \emph{directed push} to $j$'s DDM evidence accumulator, independent of $j$'s current observations:

\begin{equation}\label{eq:signal_push}
\text{signal\_push}_j = \Phi \times (1 - C_j) \times \gamma_{\text{signal}} \times \text{dir}(r_i), \quad \text{dir}(A) = +1,\; \text{dir}(B) = -1
\end{equation}

The push is \emph{additive} (not multiplicative) and enters the receiver $j$'s DDM update in Eq.~\ref{eq:ddm}. It is confidence-gated ($1 - C_j$: uncertain receivers are pushed more strongly) and pressure-scaled ($\Phi$: the environment determines whether and how strongly enforcement signals propagate). Each agent receives at most one enforcement signal per tick (from its matched partner), preserving the local interaction structure. When $\Phi = 0$, $\text{signal\_push}_j = 0$ for all agents regardless of other conditions.
...
A normative signal is stronger than a behavioural observation because it carries explicit prescriptive content (``you \emph{should} do $X$''), not merely descriptive information (``someone did $X$''). The signal comes from the agent's direct interaction partner---the only agent who directly observed the violation---making the enforcement context unambiguous.
...
\begin{center}
High-confidence agents behave consistently $\to$ low-confidence agents observe consensus $\to$ DDM evidence accumulates $\to$ norms crystallise $\to$ agents follow norms $\to$ more consistency $\to$ agents gain confidence $\to$ norm-holders paired with violators \textbf{signal partner} $\to$ partner's DDM pushed toward crystallisation $\to$ gradual cascade across pairings
\end{center}

\textbf{Source}: observations feeding normative memory (DDM) + violation-triggered partner-directed enforcement. This loop \emph{emerges} from the dual-memory architecture---it is not a separate mechanism. Because enforcement is partner-directed, the cascade propagates through sequential pairwise encounters: each enforcing agent converts at most one partner per tick.

\textbf{Requires} $V > 0$ and $\Phi > 0$: when $V = 0$, consensus observation is limited to the direct partner, making DDM drift noisy and slow; when $\Phi = 0$, partner-directed enforcement is absent. Both conditions must hold for the full normative pressure loop. Setting $V$ or $\Phi$ to zero provides clean ablation conditions for isolating the contributions of social observation and enforcement respectively (Section~\ref{sec:ablation}).
...
\begin{itemize}[itemsep=2pt]
    ...
    \item $V > 0,\; \Phi = 0$ (\emph{observation only}): Social observation accelerates DDM crystallisation; Loop~2 active. But no enforcement signalling (Loop~3 incomplete): strong-norm agents who observe violations accumulate anomalies instead of signalling their partner, making norms fragile. Expected max: Level~3--4.
    \item $V = 0,\; \Phi > 0$ (\emph{enforcement only}): Partner-directed enforcement exists but is limited---agents only observe their direct partner, so enforcement triggers only through the matched pair. Expected: marginal improvement over the isolated condition.
...
\centering
\small
\adjustbox{max width=\linewidth}{\begin{tabular}{llllp{4.5cm}}
\toprule
...
Signal amplification & $\gamma_{\text{signal}}$ & 2.0 & Both & partner-directed push magnitude; --- \\
\midrule
...
\adjustbox{max width=\linewidth}{\begin{tabular}{llp{4cm}p{4.5cm}}
\toprule
...
Crisis operator (Eq.~\ref{eq:crisis}) & 2 & Non-linear norm erosion under sustained violations \citep{centola2018tipping} & Multiplicative decay $\sigma \!\times\! \lambda$; compare gradual erosion \\
Strengthening rate $\alpha_\sigma$ & 2 & Norms deepen with practice \citep{gintis2003internalization} & Exact rate; saturating form \\
\midrule
Signal push (Eq.~\ref{eq:signal_push}) & 3 & Prescriptive signals stronger than descriptive \citep{deutsch1955study}; partner-directed to preserve locality & Additive push vs.\ drift multiplier; $\gamma_{\text{signal}}$ value \\
...
\adjustbox{max width=\linewidth}{\begin{tabular}{p{2.4cm}p{3.0cm}p{3.0cm}p{2.3cm}p{2.3cm}}
\toprule
...
No enforcement (Loop~3 incomplete; H4 fails) &
Push reversed: enforcement \emph{inhibits} crystallisation &
$\gamma\!=\!20$: single partner signal $\to$ instant crystallisation &
...
\midrule
Social pressure $\Phi$ &
$\Phi\!=\!0$: no partner enforcement (Loop~3 incomplete); norms fragile &
Enforcement pushes partner toward \emph{wrong} strategy &
$\Phi\!=\!10$: single partner signal $\to$ instant crystallisation &
...
\begin{itemize}[itemsep=4pt]
    ...
    \item \textbf{Norms are inductively derived from frequencies.} The norm's content is determined by the DDM drift $(1 - C_i)(f_A - f_B)$, which is a function of observed behavioural frequencies. The model provides a psychologically motivated mechanism for \emph{why} agents transition from descriptive to prescriptive representations (meta-cognitive uncertainty gates norm uptake), and ensures that norms, once formed, function differently from beliefs (discrete rule, anomaly-tracked, compliance-governed). However, it does not introduce normative inputs independent of behavioural observation---such as explicit values, legitimacy judgments, or moral reasoning. The is--ought bridge in the model is an uncertainty-mediated inductive process, not a deduction from independent normative premises. Whether this is sufficient to generate genuinely ``normative'' expectations in Bicchieri's sense, or merely a more elaborate route to the same frequency-based conventions, is an empirical question that the 2$\times$2 factorial is designed to test.
