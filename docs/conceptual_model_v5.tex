...
\begin{abstract}
We present a dual-memory model of norm emergence in coordination games. Agents maintain two qualitatively distinct memory systems: an \emph{experience memory} that tracks interaction history through a confidence-modulated sliding window, and a \emph{normative memory} that stores internalised rules as decision constraints. The two systems are bridged by a single state variable---predictive confidence---which measures the agent's confidence in the predictability of the social environment and governs cognitive adaptation: it controls the experience memory window (cognitive lock-in) and determines susceptibility to norm internalisation (low confidence $\to$ accept external rules). Norm formation follows a \emph{signed} two-boundary drift-diffusion process grounded in \citet{germar2014social}: uncertain agents accumulate strategy-directional stochastic evidence until a crystallisation threshold is crossed, with opposing observations cancelling rather than reinforcing. Norms strengthen through repeated conforming observations and erode through anomaly-triggered crises. Norm enforcement is violation-triggered and gated by norm strength $\sigma_i$, with enforcement signals delivered as directed pushes toward the enforced strategy, consistent with \citet{fehr2002altruistic} and \citet{toribio2023proof}. The three feedback loops of the model---individual learning, social amplification, and normative pressure---emerge naturally from the dual-memory architecture without requiring separate mechanisms.
\end{abstract}
...
To ensure reproducibility and avoid implementation ambiguity, each tick follows a fixed pipeline:
\begin{enumerate}[itemsep=2pt]
    \item \textbf{Pair and action} (\texttt{pair\_and\_action}): random matching; each pair chooses actions and interacts.
    \item \textbf{Observe and memory update} (\texttt{observe\_and\_memory\_update}): direct partner outcomes enter experience memory; optional social observations are sampled.
    \item \textbf{Predictive-confidence update} (\texttt{confidence\_update}): $C_i$ updates from prediction correctness.
    \item \textbf{Normative update} (\texttt{normative\_ddm\_or\_anomaly}): if no norm, DDM evidence accumulates; if norm exists, anomalies are counted.
    \item \textbf{Enforcement signal broadcast} (\texttt{enforcement\_signal\_broadcast}): high-strength norm holders detecting violations broadcast one-shot normative signals.
    \item \textbf{Metrics and convergence} (\texttt{metrics\_and\_convergence}): update coverage, coordination, norm-level, and convergence trackers.
\end{enumerate}
All interaction outcomes are first collected for the full population, then updates are applied in synchronized passes (experience memory pass, then confidence pass), avoiding within-tick order effects from pair processing order.

Figure~\ref{fig:tick_order_v5} shows the tick-level execution graph used by the implementation.

\begin{figure}[h]
\centering
\begin{tikzpicture}[
    node distance=7mm,
    proc/.style={draw, rounded corners, minimum width=11.8cm, minimum height=8mm, align=left, fill=gray!8, font=\small},
    arrow/.style={->, thick}
]
\node[proc] (s1) {1. Pair and action: random matching, simultaneous actions, game outcomes};
\node[proc, below=of s1] (s2) {2. Observe and memory update: partner observations + optional $k$ social observations};
\node[proc, below=of s2] (s3) {3. Confidence update: prediction correct/wrong $\Rightarrow C_i(t+1)$};
\node[proc, below=of s3] (s4) {4. Normative update: pre-crystallisation DDM \;/\; post-crystallisation anomaly tracking};
\node[proc, below=of s4] (s5) {5. Enforcement broadcast: violation-triggered signals to non-crystallised receivers};
\node[proc, below=of s5] (s6) {6. Metrics and convergence: $\% \{r_i \neq \varnothing\}$, match rate, norm level, convergence};
...
\draw[arrow] (s5) -- (s6);
\end{tikzpicture}
\caption{Fixed per-tick execution order in V5. This sequence is enforced as a deterministic update pipeline in the simulator implementation.}
\label{fig:tick_order_v5}
\end{figure}
...
\begin{itemize}[itemsep=2pt]
    ...
    \item \textbf{Two-boundary}: aligns with \citet{germar2014social}'s original experimental DDM, where perceptual judgments could shift in either direction
    \item \textbf{Correct by accumulation}: the norm strategy is determined by the direction of net evidence, not by a single-tick snapshot, making crystallisation robust to sampling noise
\end{itemize}
...
\paragraph{Evidence basis.} \citet{will2023stronger} confirmed that ``stronger norm internalisation is associated with stronger norm compliance'' and behavioural inflexibility. \citet{gintis2003internalization} showed that internalised norms create intrinsic motivation that deepens with practice. \citet{andrighetto2015agents} found that compliance becomes ``an end in itself,'' with compliance strength proportional to norm salience. These findings converge on a clear pattern: norms that are repeatedly validated by experience grow stronger over time.
...
\begin{enumerate}[itemsep=2pt]
    ...
    \item \textbf{Normative memory}: the same observations are checked for consensus (rule detection) or validity (anomaly/conformity tracking)
    \begin{itemize}
        \item If $r_i = \varnothing$: signed consistency $(f_A - f_B)$ feeds the DDM evidence accumulator (Eq.~\ref{eq:ddm})
        \item If $r_i \neq \varnothing$: violations increment the anomaly counter (Eq.~\ref{eq:anomaly}); conforming observations strengthen the norm (Eq.~\ref{eq:strengthen})
    \end{itemize}
\end{enumerate}
...
Agents maintain two memory systems bridged by predictive confidence. \textbf{Experience memory} stores interaction history in a confidence-modulated sliding window, producing statistical beliefs; this creates \emph{cognitive lock-in} where successful coordination stabilises beliefs through expanded memory. \textbf{Normative memory} stores internalised rules as decision constraints, formed through a signed two-boundary drift-diffusion process \citep{germar2014social,germar2019learning} when uncertain agents observe consistent behaviour---with opposing evidence cancelling rather than reinforcing, and norms forming only when genuine consensus exists. Once formed, norms are strengthened by conforming observations and maintained through anomaly accumulation (not statistical updating); they are overthrown only through crisis-induced phase transitions. Predictive confidence determines whether an agent is a \emph{norm receiver} (low confidence $\to$ cannot predict the environment, so internalises others' consistency as a rule) or a \emph{norm follower} (moderate/high confidence $\to$ norm provides a useful constraint supplementing experience). Enforcement is gated by normative conviction: agents with high norm strength $\sigma_i$ treat violations as errors to be corrected (broadcasting directed enforcement signals), while low-$\sigma_i$ agents accumulate anomalies. The three feedback loops---cognitive lock-in, social amplification, and normative pressure---emerge naturally from this architecture. The result: a model where experience memory explains norm \emph{stability}, normative memory explains norm \emph{prescriptive force}, and their interaction produces self-enforcing social norms from cold start.
...
\text{DDM:} && e(t\!+\!1) &= e(t) + (1\!-\!C)(f_A\!-\!f_B) + \text{push} + \mathcal{N}(0,\sigma^2) && \text{(signed, two-boundary)} \\
\text{Crystallise:} && |e| &\geq \theta \;\Rightarrow\; r = A \text{ if } e>0,\; B \text{ if } e<0 && \text{(direction from accumulation)} \\
\text{Strengthen:} && s^*\!=\!r &\;\Rightarrow\; \sigma \leftarrow \min(1,\; \sigma + \alpha_\sigma(1\!-\!\sigma)) && \text{(conformity reinforcement)} \\
...
\text{Signal push:} && \text{push} &= (1\!-\!C) \cdot \gamma \cdot \text{dir}(s_{\text{enforced}}) && \text{(directed, confidence-gated)} \\
...
\paragraph{Scope of predictive confidence.} Predictive confidence $C_i$ governs two pre-crystallisation processes: (1)~the experience memory window (Eq.~\ref{eq:window}), determining how much past data the agent retains, and (2)~the DDM drift rate (Eq.~\ref{eq:drift}), determining how quickly the agent absorbs norm evidence (``copy when uncertain''). It also gates enforcement signal reception (Eq.~\ref{eq:signal_push}): uncertain agents are pushed more strongly by directed signals. It does \emph{not} govern post-crystallisation norm compliance. Once a norm has crystallised, compliance is determined by norm strength $\sigma_i$ (Eq.~\ref{eq:compliance}), which evolves through conformity strengthening (Eq.~\ref{eq:strengthen}), anomaly accumulation, and crisis dynamics. This separation reflects the distinction between epistemic uncertainty (``can I predict?'') and normative conviction (``how binding is the rule?'')---an agent may be highly confident in predicting the environment \emph{because} it follows a strong norm, and that confidence should not undermine the very compliance that produces it.
