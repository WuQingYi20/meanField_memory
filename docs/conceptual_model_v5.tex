...
\node[step, fill=memblue]     (s2) at (0,-1.2)  {2.\ Observe and Memory Update};
...
\node[step, fill=normyellow]   (s5) at (0,-4.8)  {5.\ Enforcement Broadcast};
...
\node[var] at (5.8,-1.2)  {\textbf{Write:} $M^E_i$, $\mathbf{b}^{\text{exp}}$};
...
\node[var] at (5.8,-4.8)  {\textbf{Write:} $p_i$ broadcast (Eq.~\ref{eq:signal_push})};
\node[var] at (5.8,-6.0)  {\textbf{Read:} all; no state written};
\end{tikzpicture}
\caption{Single-tick dataflow. Left: execution stages in fixed order. Right: variables written/read at each stage, with equation references. Use this figure to trace where any symbol first appears and where it is consumed.}
...
We measure norm emergence on three continuous layers that map to Bicchieri's taxonomy:
\begin{enumerate}[itemsep=2pt]
    \item \textbf{Behavioral layer} $\leftrightarrow$ descriptive norm (behavioural regularity): majority fraction $\geq 0.95$.
    \item \textbf{Belief layer} $\leftrightarrow$ empirical expectations (experience memory accuracy, using $b_{\mathrm{exp}}$ not $b_{\mathrm{eff}}$): mean belief error $< 0.10$ and belief variance $< 0.05$.
    \item \textbf{Crystallisation layer} $\leftrightarrow$ normative expectations (norm internalisation via DDM): $\geq 80\%$ crystallised with $\geq 90\%$ holding the dominant norm.
\end{enumerate}
Convergence = all three layers past thresholds for 50 consecutive ticks. The crystallisation layer requires normative memory.

\begin{itemize}[itemsep=2pt]
    \item \emph{Baseline}: Slowest convergence. Behavioral and belief layers only; never converges (crystallisation blocked).
    \item \emph{Normative only}: Norms form but lack lock-in stability. All layers reachable but fragile.
    \item \emph{Lock-in only}: Stable conventions, not norms. Behavioral and belief layers only; never converges.
    \item \emph{Full model}: Fastest and most stable. All three layers converge.
\end{itemize}
...
\bottomrule
\end{tabular}
\caption{Full parameter list with defaults, layer assignment, and provenance.}
\label{tab:params}
\end{table}
