...
The upper bound $\text{max} = 6$ is grounded in empirical findings on experience-based decision making. \citet{hertwig2010decisions} showed that in decisions from experience, people rely on surprisingly small samples of recent outcomes, with recency effects further shrinking the effective sample. The BI-SAW model \citep{nevo2012bi}, which won the 2010 Market Entry Prediction Competition by best fitting human behavior in repeated games, found that agents recall only the most recent \textbf{6 trials} of past experience---a bounded memory parameter estimated from experimental data. We adopt this value as our upper bound.
...
Memory base window & base & 2 & \citet{hertwig2010decisions} & Individual \\
Memory max window & max & 6 & \citet{nevo2012bi} & Individual \\
\midrule
